...
(function () {
  ...
  var DEFAULT_ESCRITORIOS = [
    { cidade:'Porto-Pi',                      endereco:'Av. Presidente Vargas, 193', cep:'64145-000', uf:'PI', horario:'Seg–Sex: 08h–12h / 14h–17h | Sáb: 08h–12h | Dom: Fechado', maps:'https://maps.app.goo.gl/jKtTqEDryhaYBsZL6', foto:'img/escritorioporto.png' },
    { cidade:'Nossa Senhora dos Remédios-Pi', endereco:'Rua Marcelino Rodrigues',   cep:'64149-000', uf:'PI', horario:'Seg–Sex: 08h–12h / 14h–17h | Sáb: 08h–12h | Dom: Fechado', maps:'https://maps.app.goo.gl/448sHGHJaQ4pTbSt5', foto:'img/escritorionsr.png' },
    { cidade:'Miguel Alves-Pi',               endereco:'R. São José, 85',           cep:'64130-000', uf:'PI', horario:'Seg–Sex: 08h–12h / 14h–17h | Sáb: 08h–12h | Dom: Fechado', maps:'https://maps.app.goo.gl/HaHjPuttG7NBzihg8', foto:'img/escritoriomiguelalves.png' },
    { cidade:'Barras-Pi',                     endereco:'Av. Juscelino Kubitscheck', cep:'64100-000', uf:'PI', horario:'Seg–Sex: 08h–12h / 14h–17h | Sáb: 08h–12h | Dom: Fechado', maps:'https://maps.app.goo.gl/ha35JvA1NLwjoLMj7', foto:'img/escritoriobarras.png' }
  ];
  ...
  function renderizarEscritorios() {
    ...
    grid.innerHTML = lista.map(function(e, i) {
      ...
      var fotoHtml = e.foto
        ? '<div style="height:220px;overflow:hidden;border-radius:12px 12px 0 0;background:linear-gradient(135deg,#2B5FDB,#00D4FF);display:flex;align-items:center;justify-content:center;"><img src="' + escHtml(fotoSrc) + '" alt="' + escHtml(e.cidade||'') + '" loading="lazy" style="width:100%;height:100%;object-fit:contain;transition:transform .4s;" onmouseover="this.style.transform=\'scale(1.05)\'" onmouseout="this.style.transform=\'scale(1)\'" onerror="this.style.display=\'none\'"></div>'
        : '<div style="height:220px;border-radius:12px 12px 0 0;background:linear-gradient(135deg,#2B5FDB,#00D4FF);display:flex;align-items:center;justify-content:center;">' + PIN_SVG.replace('class="w-6 h-6 text-white"','style="width:40px;height:40px;color:#fff;opacity:.6"') + '</div>';
  ...
  function carregarJsonGitHub(arquivo, chaveLS, callback) {
    fetch(BASE_RAW + arquivo + '?t=' + Date.now())
      .then(function(r){ return r.ok ? r.json() : null; })
      .then(function(dados){
        if (dados) lsSet(chaveLS, dados);
        if (callback) callback(dados);
      })
      .catch(function(){ if (callback) callback(null); });
  }
  ...
  function carregarDatasGitHub(callback) {
    fetch(BASE_RAW + 'datas.json?t=' + Date.now())
      .then(function(r){ return r.ok ? r.json() : null; })
      .then(function(datas){
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    carregarJsonGitHub('vantagens.json',   'vantagens',   function(d){ if(d) renderizarVantagens(d); });
