...
(function () {
  ...
  var DEFAULT_ESCRITORIOS = [
    ...
    { cidade:'Barras-Pi',                     endereco:'Av. Juscelino Kubitscheck', cep:'64100-000', uf:'PI', horario:'Seg–Sex: 08h–12h / 14h–17h | Sáb: 08h–12h | Dom: Fechado', maps:'https://maps.app.goo.gl/ha35JvA1NLwjoLMj7', foto:'img/escritoriobarras.jpeg' }
  ];
  ...
  function renderizarEscritorios() {
    ...
    grid.innerHTML = lista.map(function(e, i) {
      ...
      var fotoHtml = e.foto
        ? '<div style="height:160px;overflow:hidden;border-radius:12px 12px 0 0;background:linear-gradient(135deg,#2B5FDB,#00D4FF);"><img src="' + escHtml(fotoSrc) + '" alt="' + escHtml(e.cidade||'') + '" loading="lazy" style="width:100%;height:100%;object-fit:cover;transition:transform .4s;" onmouseover="this.style.transform=\'scale(1.05)\'" onmouseout="this.style.transform=\'scale(1)\'" onerror="this.style.display=\'none\'"></div>'
        : '<div style="height:160px;border-radius:12px 12px 0 0;background:linear-gradient(135deg,#2B5FDB,#00D4FF);display:flex;align-items:center;justify-content:center;">' + PIN_SVG.replace('class="w-6 h-6 text-white"','style="width:40px;height:40px;color:#fff;opacity:.6"') + '</div>';
